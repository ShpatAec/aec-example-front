...
function Home() {
    ...
    return (
        <div className="container">
            <h1 className="text-2xl font-bold text-indigo-400">Rest Api Project Test</h1>
            ...
            {fetchState === FetchState.SUCCESS && (
                <>
                    <p className="mb-4 mt-4 text-2xl font-bold tracking-tight text-zinc-600 dark:text-white">Posts:</p>
                    <ul className="grid grid-cols-4 gap-2">
                        {posts.map((post) => (
                            <li key={post.id}
                                className="col-span-2 block max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700 mb-4">
                                <h3 className="mb-2 text-2xl font-bold tracking-tight text-blue-600 dark:text-white">
                                    {post.id} - {post.title}
                                </h3>
                                <p className="indent-6 italic text-violet-400">{post.body}</p>
                            </li>
                        ))}
                    </ul>
                </>
